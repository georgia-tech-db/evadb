...
from udf_faster_rcnn.lib.model.roi_crop.functions.roi_crop import RoICropFunction
import cv2
import pdb
import random
...
def vis_detections(img_name,im, class_name, dets, thresh=0.8):
    ...
    for i in range(np.minimum(10, dets.shape[0])):
        ...
        if score > thresh:
            ...
            file.write(img_name+","+class_name+","+str(bbox[0])+","+str(bbox[1])+","+str(bbox[2])+","+str(bbox[3])+"\n")
    return im
