...
class DepthEstimator(AbstractClassifierUDF):
    ...
    UDF class for depth estimation feature. responsible for taking batch of
    frames. It processes the frames, sends them to deep learning model for
    predictions. Consolidates the model output and store them in
    DepthEstimationResult object.
    Arguments:
        Takes AbstractClassifierUDF as input since it implements the
        abstract UDF class.
    ...
    def _get_depth_estimation(
            self, frames: np.ndarray) -> Tuple[List[List[float]],
                                               List[List[float]]]:
        """
        Performs depth and segmentation predictions on input frames
        Arguments:
            frames (np.ndarray): Frames on which predictions need to be
            performed

        Returns:
            tuple containing predicted_segments (List[List[float]]),
            predicted_depths (List[List[float]])
        ...
        for i, img in enumerate(frames):
            ...
            img_var = Variable(
                torch.from_numpy(
                    self.prepare_img(img).transpose(
                        2,
                        0,
                        1)[None]),
            ...
            segm =
            cv2.resize(segm[0, :NUM_CLASSES].cpu().data.numpy().transpose(
