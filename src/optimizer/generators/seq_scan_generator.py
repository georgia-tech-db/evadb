...
from src.optimizer.operators import LogicalGet, Operator, LogicalFilter, \
    LogicalProject, LogicalUnion, LogicalOrderBy, LogicalLimit, LogicalExplode
...
class ScanGenerator(Generator):
    ...
    def _visit_logical_explode(self, operator: LogicalExplode):
        explode_plan = ExplodePlan(operator.column_list)
    ...
    def _visit(self, operator: Operator):
        ...
        if isinstance(operator, LogicalExplode):
            self._visit_logical_explode(operator)
