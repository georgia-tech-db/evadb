...
class PP:
    def __init__(self):
        ...
        # KDE, SVM, NN - this should be a mapping of model name to model
        # CONSTRUCTOR
        self.model_library = {
            "kde": self._kde,
            "svm": self._svm,
        ...
        # save the statistics related to the model, although most stats are
        ...
        for c in X:
            if c == "speed":
                ...
                for item in sub_labels:
                    label_dict["i=" + item] = []
                    label_dict["o=" + item] = []
                # column_of_interest would be vehicle type; data would be
                # ("car", "van", "car")
                for data in column_of_interest:
                    for item in sub_labels:
    ...
    def train_all(self, image_matrix, data_table):
        ...
        X_train, X_val, label_dict_train, label_dict_val = self._split_train_val(
    ...
    def _evaluate(self, X_test, label_dict):
        ...
        for category_name in self.category_library:
            for model_name in self.category_library[category_name]:
                ...
                self.category_stats[category_name][model_name]["R"] = 1 - \
                    float(sum(y_hat)) / len(y_hat)
    ...
    def _rf(self, args):
        ...
        for label in label_dict:
            ...
            self.category_stats[label][pre +
                                       "/rf"] = {"C": round(time.time() -
                                                            tic +
                                                            self.pre_category_stats[pre]["C"], 2)}  #
    ...
    def _pca(self, args):
        ...
        # we will return the models for the caller to save them
        return [X_new, label_dict]
