...
if __name__ == "__main__":
    ...
    video_dir_list = sorted(os.listdir(data_dir))
    ...
    mmanager.add_frame_mapping(
        conn,
        loader.video_start_indices,
    ...
    s = time.time()
    images_list = operator.transform(
        ["grayscale", "blur"], images_list, {"kernel_size": 5}
    )
    loader.update_images(frame_ids, images_list, False)
    e = time.time()
    print((e - s) / len(images_list))
