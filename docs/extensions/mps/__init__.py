...
def mps_tag_role(name, rawtext, text, lineno, inliner, options={}, content=[]):
    try:
        ...
        return [], [
            inliner.document.reporter.warning("mps:tag without mps:prefix", line=lineno)
        ]
    if len(text) == 0:
        return [], [
            inliner.document.reporter.error("missing argument for mps:tag", line=lineno)
        ]
    targetnode = nodes.target("", "", ids=[targetid])
    tag = ".{}:".format(text)
    refnode = nodes.reference(
        "", "", refid=targetid, classes=["mpstag"], *[nodes.Text(tag)]
    )
