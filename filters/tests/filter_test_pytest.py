...
def test_get_vehtype_labels(filter,label_path):
    ...
    if (y[0]!=1):
        assert False
    assert True
    #TODO: Create test cases for ending frames absent in xml and intermediate frames absent in xml. Small Data passes these two conditions all the time.
...
def test_rf():
    ...
    try:
        ...
        if obj.category_library['veh_type']['none/rf']==None or not isinstance(obj.category_library['veh_type']['none/rf'],RandomForestClassifier) :
            assert False
...
def test_dnn():
    ...
    try:
        ...
        if obj.category_library['veh_type']['none/dnn']==None or not isinstance(obj.category_library['veh_type']['none/dnn'],MLPClassifier) :
            assert False
    except Exception as e:
        assert False
    #obj.rf()
...
def test_pca():
    ...
    try:
        n_components=135
        li=obj.pca([X.reshape(X.shape[0],-1),{'veh_type':y},n_components])
        X=li[0]
