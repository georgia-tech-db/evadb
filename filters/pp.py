...
class PP_gen:
  ...
  def __init__(self):
    ...
    """The way to use these libraries would be something like
    ...
    self.model_library = {"kde": self.kde,
                          "svm": self.svm,
                          "dnn": self.dnn} #KDE, SVM, NN - this should be a mapping of model name to model CONSTRUCTOR

    self.model_library_pre = {"fh": self.feature_hashing,
                              "pca": self.pca,
                              "sampling": self.sampling} #feature hashing, PCA, None - Separated to do mix and match
  ...
  def pca(self, args):
    ...
    return pca #we will return the models for the caller to save them
