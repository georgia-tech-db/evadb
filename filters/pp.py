...
class PP:
    def __init__(self):
        ...
        self.model_library = {"kde": self._kde,
                              "svm": self._svm,
                              "dnn": self._dnn,
                              "rf": self._rf}  # KDE, SVM, NN - this should
        # be a
        # mapping of model name to model CONSTRUCTOR

        self.pre_model_library = {"none": self._none,
                                  "pca": self._pca,
                                  # "fh": self.feature_hashing,
                                  # "sampling": self.sampling,
                                  }  # feature hashing, PCA, None -
        # Separated to
        # do mix and match

        # self.pre_results = {} #save the preprocessed results {
        # "pre_model_name": reformed_data
        self.category_library = {}  # save the trained model
        self.category_stats = {}  # save the statistics related to the model,
        # although most stats are embedded in the model,
    ...
    def _generate_binary_labels(self, X):
        """
        Example label dict is going to be {"car": [0,0,0,1,0,0,0.....],
        "others": [0,1,0,0,0...}
        :param X:
        :return:
        """
        ...
        for c in X:
            if c == "speed":
                ...
                for item in sub_labels:
                    label_dict["i=" + item] = []
                    label_dict["o=" + item] = []
                for data in column_of_interest:  # column_of_interest would be
                    # vehicle type; data would be ("car", "van", "car")
                    for item in sub_labels:
    ...
    def _reshape_image(self, X):
        print(('inside reshape images, shape of image dataseries is ' + str(
            X.shape)))
        reduction_rate = 12
    ...
    def train_all(self, image_matrix, data_table):
        ...
        X_train, X_val, label_dict_train, label_dict_val = \
            self._split_train_val(
                X_preprocessed, label_dict)
    ...
    def _process(self, X, label_dict):
        for process_method in X:
            for model in self.model_library:
                self.model_library[model](
                    [X[process_method], label_dict, process_method])

    def _preprocess(self, X, label_dict):
        X_preprocessed = {}
        for model in self.pre_model_library:
            X_preprocessed[model], _ = self.pre_model_library[model](
                [X, label_dict])
    ...
    def _evaluate(self, X_test, label_dict):
        ...
        self.category_stats[category_name] = {model_name: {"reduction_rate":
        model.score(), "false_negative_rate": model......., "time_to_train":}
        """

        for category_name in self.category_library:
            for model_name in self.category_library[category_name]:
                # We need to parse by "/" token and apply the proper
    ...
    def _rf(self, args):
        ...
        for label in label_dict:
            ...
            self.category_stats[label][pre + "/rf"] = {"C": round(
                time.time() - tic + self.pre_category_stats[pre]["C"], 2)}  #
    ...
    def _dnn(self, args):
        ...
        for label in label_dict:
            ...
            if label not in self.category_stats:
                self.category_stats[label] = {}
            self.category_stats[label][pre + "/dnn"] = {"C": round(
                time.time() - tic + self.pre_category_stats[pre]["C"], 2)}
    ...
    def _svm(self, args):
        ...
        for label in label_dict:
            ...
            if len(np.unique(label)) == 1:
                ...
                if label not in self.category_stats:
                    self.category_stats[label] = {}
                self.category_stats[label][pre + "/svm"] = {"C": round(
                    time.time() - tic + self.pre_category_stats[pre]["C"], 2)}
        return
    ...
    def _kde(self, args):
        ...
        for label in label_dict:
            ...
            if label not in self.category_stats:
                self.category_stats[label] = {}
            self.category_stats[label][pre + "/kde"] = {"C": round(
                time.time() - tic + self.pre_category_stats[pre]["C"], 2)}
    ...
    def _pca(self, args):
        ...
        if "pca" not in self.pre_category_library:
            ...
            if "pca" not in self.pre_category_stats:
                self.pre_category_stats["pca"] = {
                    "C": round(time.time() - tic, 2)}
        else:
            pca = self.pre_category_library["pca"]
            X_new = pca.transform(X)

        return [X_new,
                label_dict]  # we will return the models for the caller to
        # save them

    # According to paper, each pixel or blob of 8x8 can be mapped to a
    # dimension
