...
then
    ...
    then
        PYTHONPATH=./ pytest --durations=20 -n 4 --cov-report term-missing:skip-covered  --cov-config=.coveragerc --cov-context=test --cov=eva/ --capture=sys --tb=short -v --log-level=WARNING -m "not benchmark"
    elif [[ "$MODE" = "RAY" ]];
    then
