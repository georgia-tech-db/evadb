...
then
    ...
    then
        PYTHONPATH=./ pytest --cov-report term-missing:skip-covered  --cov-config=.coveragerc --cov-context=test --cov=eva/ -s -v --log-level=WARNING -m "not benchmark and not notparallel" --dist loadscope -n auto
    elif [[ "$MODE" = "RAY" ]];
    then
