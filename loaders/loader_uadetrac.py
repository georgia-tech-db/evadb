...
parser.add_argument('--save_path', default='npy_files',
...
class LoaderUADetrac(LoaderTemplate):
    ...
    def save_images(self):
        save_dir = os.path.join(self.eva_dir, 'data', args.save_path,
                                'ua_detrac_images.npy')
                ...
                for frame in tree_root.iter('frame'):
                    ...
                    intersection_per_frame = \
                        self.task_manager.call_intersection(
                            self.images[curr_frame_num - 1], scene,
                            bboxes)
    ...
    def load_from_csv(filename):
        ...
        converters = {"image": LoaderUADetrac().image_eval,
                      "vehicle_type": eval, "color": eval,
                      "intersection": eval}
