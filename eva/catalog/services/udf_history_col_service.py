...
class UdfHistoryColumnService(BaseService):
    ...
    def get_cols_by_udf_history_id(self, udf_history_id: int) \
            -> List[int]:
        ...
        result = self.model.query \
            .with_entities(self.model._arg) \
            .filter(self.model._udf_history_id == udf_history_id) \
            .order_by(self.model._arg_index).all()
        return result

    def create_udf_history_cols(self, udf_history_id: int,
                                cols: List[int]) -> bool:
        ...
        try:
            for idx, arg in enumerate(cols):
                udf_history_columns.append(
                    UdfHistoryColumn(udf_history_id, arg, idx)
                )
