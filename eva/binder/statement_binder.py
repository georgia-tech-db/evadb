    ...
    def bind_node(self, node: Union[AbstractStatement, AbstractExpression]):
        ...
        def _(node: TableRef):   
            if node.is_select():
                ...
                bind(node.table) 
                self._binder_context = current_context
                self._binder_context.add_derived_table_alias(node.alias, node.table.target_list)
            else:
                # Table
        ...
        def _(node: TupleValueExpression):
            table_alias, col_obj = self._binder_context.get_binded_column(node.col_name, node.table_alias)
        ...
        def _(node: FunctionExpression):
            ...
            assert udf_obj is not None, 'UDF with name {} does not exist in the catalog. Please create the UDF using CREATE UDF command'.format(node.name)
        
            output_objs = self._catalog.get_udf_outputs(udf_obj)
            if node.output:
                for obj in output_objs:
                    if obj.name.lower() == node.output:
                        node.output_col_aliases.append('{}.{}'.format(node.alias, obj.name.lower()))
                assert len(node.output_col_aliases) == 1, 'Duplicate columns {} in UDF {}'.format(node.output, udf_obj.name)
            else:
                node.output_col_aliases = ['{}.{}'.format(node.alias, obj.name) for obj in output_objs]            

            node.function = path_to_class(udf_obj.impl_file_path, udf_obj.name)()
            
            # add the column names to the binder alias map
            self._binder_context.add_function_alias(node.alias, node.name.lower())
