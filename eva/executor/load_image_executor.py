...
class LoadImageExecutor(AbstractExecutor):
    ...
    def exec(self):
        ...
        if Path(self.node.file_path).exists():
            image_file_path:'Path' = self.node.file_path
        ...
        file_count = 0
        suffix_set = {".png", ".jpeg", ".jpg"}
        print(f"\nLoading images from {image_file_path}")
        for file in tqdm(tuple(image_file_path.iterdir())):
            if file.is_file():
                # workaround to avoid reading hidden files.
                if not file.name.startswith(".") and file.suffix in suffix_set:
