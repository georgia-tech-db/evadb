...
# If the inner node of a lateral join is a function-valued expression, we eliminate the # join node and make the inner node the parent of the outer node. This produces a
# linear #data flow path. Because this scenario is common in our system, we chose to
# explicitly #convert it to a linear flow, which simplifies the implementation of other
# optimizations such as UDF reuse and parallelized plans by removing the join.
