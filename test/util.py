...
def create_sample_csv(num_frames=NUM_FRAMES):
    ...
    df_sample_meta.to_csv(os.path.join(PATH_PREFIX, "dummy.csv"), index=False)


def create_dummy_csv_batches():
    df = pd.read_csv(
        os.path.join(PATH_PREFIX, "dummy.csv"), converters={"bbox": convert_bbox}
    )
    return Batch(df)
...
def create_csv(num_rows, columns):
    ...
    df.to_csv(os.path.join(PATH_PREFIX, "dummy.csv"), index=False)
    return df
...
def create_sample_video(num_frames=NUM_FRAMES):
    ...
    for i in range(num_frames):
        frame = np.array(np.ones((2, 2, 3)) * float(i + 1)
                         * 25, dtype=np.uint8)
        out.write(frame)
...
def copy_sample_video_to_prefix():
    shutil.copyfile(
        "data/ua_detrac/ua_detrac.mp4", os.path.join(
            PATH_PREFIX, "ua_detrac.mp4")
    )
