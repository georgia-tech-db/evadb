...
class AnnotateTests(unittest.TestCase):
    ...
    def test_annotate_one_frame(self):
        imarray = np.random.randint(0, 255, size=(100, 100, 3))
        bbox = np.array([0, 0, 30, 60])
        df = pd.DataFrame([[imarray, bbox]])
        annotated_image = self.annotate_instance(df)

        expected_image = pd.DataFrame(
            [[imarray[0:60, 0:30]]], columns=["annotated_frame_array"]
        )
        self.assertTrue(expected_image.equals(annotated_image))
        pass

    def test_annotate_multiple_frames(self):
        # imarray = np.random.randint(0, 255, size=(100, 100, 3))
    ...
    def test_annotate_bad_bbox(self):
        # imarray = np.random.randint(0, 255, size=(100, 100, 3))
