...
@forward(
    ...
    output_signature=PyTorchTensor()
)
...
class UdfDecoratorTest(unittest.TestCase):
    ...
    def test_forward_fn_pytorch_no_constraints(self):
        ...
        self.assertTrue(torch.eq(forward_fn_pytorch_no_constraints(None, torch_tensor), 2))
    ...
    def test_forward_fn_pytorch_with_constraints(self):
        #all the constraints are satisfied
        torch_tensor = torch.ones((2,1))
        self.assertTrue(torch.eq(forward_fn_pytorch_input_output_constraint(None, torch_tensor), 2))
