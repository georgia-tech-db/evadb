...
class ClickHouseNativeStorageEngineTest(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.execute_native_query_patcher = patch(
            "evadb.third_party.databases.clickhouse.clickhouse_handler.ClickHouseHandler.execute_native_query"
        )
        self.execute_native_query_mock = self.execute_native_query_patcher.start()

        self.connect_patcher = patch(
            "evadb.third_party.databases.clickhouse.clickhouse_handler.ClickHouseHandler.connect"
        )
        self.connect_mock = self.connect_patcher.start()

        self.disconnect_patcher = patch(
            "evadb.third_party.databases.clickhouse.clickhouse_handler.ClickHouseHandler.disconnect"
        )
