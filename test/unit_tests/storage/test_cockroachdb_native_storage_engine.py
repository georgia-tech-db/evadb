...
class CockroachDBNativeStorageEngineTest(unittest.TestCase):
    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)

    def get_cockroachdb_params(self):
        return {
            "user": "eva",
            "password": "password",
            "host": "localhost",
    ...
    def setUp(self):
        connection_params = self.get_cockroachdb_params()
        ...
        self.execute_native_query_patcher = patch(
            "evadb.third_party.databases.cockroachdb.cockroachdb_handler.CockroackDBHandler.execute_native_query"
        )
        self.execute_native_query_mock = self.execute_native_query_patcher.start()

        self.connect_patcher = patch(
            "evadb.third_party.databases.cockroachdb.cockroachdb_handler.CockroackDBHandler.connect"
        )
        self.connect_mock = self.connect_patcher.start()

        self.disconnect_patcher = patch(
            "evadb.third_party.databases.cockroachdb.cockroachdb_handler.CockroackDBHandler.disconnect"
        )
    ...
    def test_execute_cockroachdb_select_query(self):
    ...
    def test_execute_cockroachdb_insert_query(self):
        execute_query_fetch_all(
            self.evadb,
            """USE test_data_source {
    ...
    def test_execute_cockroachdb_update_query(self):
        execute_query_fetch_all(
            self.evadb,
            """USE test_data_source {
                UPDATE test_table
    ...
    def test_execute_cockroachdb_delete_query(self):
        execute_query_fetch_all(
            self.evadb,
            """USE test_data_source {
