...
class DropExecutorTest(unittest.TestCase):
    ...
    def test_should_drop_table(self):
        ...
        table_catalog_entry = self.evacatalog().get_table_catalog_entry("MyVideo")
        video_dir = table_catalog_entry.file_url
        self.assertFalse(table_catalog_entry is None)
        column_objects = self.evacatalog().get_column_catalog_entries_by_table(
            table_catalog_entry
        )
        ...
        video_metadata_table = (
            self.evacatalog().get_multimedia_metadata_table_catalog_entry(
                table_catalog_entry
            )
        )
        ...
        self.assertTrue(self.evacatalog().get_table_catalog_entry("MyVideo") is None)
        column_objects = self.evacatalog().get_column_catalog_entries_by_table(
            table_catalog_entry
        )
...
class DropUDFExecutorTest(unittest.TestCase):
    ...
    def test_should_drop_udf(self):
        ...
        udf = self.evacatalog().get_udf_catalog_entry_by_name(udf_name)
    ...
    def test_drop_wrong_udf_name(self):
        ...
        udf = self.evacatalog().get_udf_catalog_entry_by_name(right_udf_name)
        self.assertTrue(udf is not None)
