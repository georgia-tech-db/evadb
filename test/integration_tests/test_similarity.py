...
class SimilarityTests(unittest.TestCase):
    ...
    def test_reddit_data(self):
        ...
        create_query = """CREATE MATERIALIZED VIEW reddit_dataset_object (name, data, bboxes, labels)
                          AS SELECT name, data, bboxes, labels FROM reddit_dataset JOIN LATERAL UNNEST(YoloV5(data)) AS Obj(labels, bboxes, scores);"""
        execute_query_fetch_all(create_query)

        # select_query = """SELECT bbox, label FROM reddit_dataset JOIN LATERAL YoloV5(data) AS Obj(label, bbox, score);"""
        # select_query = """SELECT _row_id, name, data, bboxes, labels FROM reddit_dataset_object"""
        # res_batch = execute_query_fetch_all(select_query)
        # for i in range(len(res_batch)):
        #     print(res_batch.frames["reddit_dataset_object._row_id"].iloc[i])
        #     print(res_batch.frames["reddit_dataset_object.name"].iloc[i])
        #     print(res_batch.frames["reddit_dataset_object.labels"].iloc[i])
        #     print(res_batch.frames["reddit_dataset_object.bboxes"].iloc[i])
        ...
        select_query = """SELECT name FROM reddit_dataset_object
                            ORDER BY Similarity(
