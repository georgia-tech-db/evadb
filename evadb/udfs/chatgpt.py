...
class ChatGPT(AbstractUDF):
    ...
    Input Signatures:
        ...
        prompt (str) :  An optional prompt that can be passed to the model. It can contain instructions to the model,
                        ...
                        If not provided, the system prompt defaults to a helpful assistant that accomplishes user tasks.
    ...
    Example Usage:
        ...
        In the above UDF invocation, the 'query' passed would be the user task to generate video summaries, the 'content'
        passed would be the video transcripts that need to be used in order to generate the summary. Since no prompt
        is passed, the default system prompt will be used.
