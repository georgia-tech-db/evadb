...
TODO: Currently there seems to be a importing issue that I am not sure how to solve
...
class QOMinimum(QOTemplate):
  ...
  def __init__(self):
    ## later add support for pyparsing.. could definitely help with parsing operations
    self.operators = ["!=", ">=", "<=", "=", "<", ">"]
    self.separators = ["||", "&&"]
  ...
  def executeQueries(self, queries:list):
    ...
    for query in queries:
      execution_plans.append( self.run(query, synthetic_pp_list, synthetic_pp_stats, label_desc) )
